...
***********************************************************************************************************************/
/*********************************************************************************************************************
 * File Name    : r_lpc_rx14t.c
 * Description  : Implementation of the LPC module for the RX14T
 **********************************************************************************************************************/
/*********************************************************************************************************************
...
static uint32_t hoco_frequency[4];     // Possible HOCO frequencies;
...
static lpc_err_t lpc_freq_range_check (lpc_operating_mode_t e_mode)
{
    ...
    clock_frequency[3] = 0;                // SUBCLK not used in RX14T
    clock_frequency[4] = 0;                // PLL
    clock_frequency[5] = 15000;            // IWDT - 15 kHz

    hoco_frequency[0] = 64000000; // 64 MHz
    hoco_frequency[1] = 48000000; // 48 MHz       
    hoco_frequency[2] = 24000000; // 24 MHz 
    hoco_frequency[3] = 32000000; // 32 MHz   
...
static uint32_t lpc_clockhzget (lpc_system_clocks_t clock)
{
    ...
    if (LPC_CLOCK_ACTIVE == SYSTEM.HOCOCR.BIT.HCSTP)
    {
        ...
        clock_frequency[LPC_CKSEL_HOCO] = hoco_frequency[OFSM.OFS1.BIT.HOCOFRQ];
    }
...
static lpc_err_t lpc_lowpower_activate_check (void)
{
    if (0x1 == SYSTEM.SBYCR.BIT.SSBY) /* check if entering software standby */
    {
        ...
        {
            return LPC_ERR_ILLEGAL;         /* Cannot enter SW_Standby if DTC is enabled */
        }
    }
