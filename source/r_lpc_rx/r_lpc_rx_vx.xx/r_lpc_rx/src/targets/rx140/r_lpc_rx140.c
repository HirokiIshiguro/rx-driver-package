...
***********************************************************************************************************************/
/*********************************************************************************************************************
 * File Name    : r_lpc_rx140.c
 * Description  : Implementation of the LPC module for the RX140
 **********************************************************************************************************************/
/*********************************************************************************************************************
...
static uint32_t hoco_frequency[4];     // Possible HOCO frequencies; Indices 1&2&3 are not used for the RX140.
...
lpc_err_t lpc_snooze_mode_configure(lpc_snooze_mode_t * snooze_mode)
{
    ...
    if (snooze_mode->snooze_release == LPC_SNZ_RESET)
    {
    ...
    switch (e_mode)
    {
        case LPC_OP_HIGH_SPEED:
            ...
            if ((((iclk_freq > LPC_MAX_HIGH_SPEED_FREQ_HZ)
               ...
               || (pclkb_freq > LPC_MAX_HIGH_SPEED_PCLKB_FREQ_HZ))
            {
        ...
        case LPC_OP_MIDDLE_SPEED_2:
            ...
            /* Check if clock speeds are within limits */
            if ((((iclk_freq > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ)
               || (fclk_freq > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
               || (pclkd_freq > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
               || (pclkb_freq > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
            {
                return LPC_ERR_CLOCK_EXCEEDED;
            }
        break;
...
static lpc_err_t lpc_lowpower_activate_check (void)
{
    ...
    if (0x1 == SYSTEM.SBYCR.BIT.SSBY) /* check if entering software standby */
    {
        ...
        if ((0 == SYSTEM.MSTPCRA.BIT.MSTPA28) && (1 == DTC.DTCST.BIT.DTCST) && (0 == SYSTEM.SNZCR.BIT.SNZDTCE))
        {
        ...
        else if (LPC_MEDIUM_SPD_2 == SYSTEM.OPCCR.BIT.OPCM) /* if return mode is medium speed mode2 */
        {
            ...
            switch(rstckcr_rstcksel)
            {
                ...
                case LPC_MAIN_OSC:  /* If Main Clock is chosen as the return clock */
                    ...
                    /* When exiting sleep-mode to medium-speed mode with the main clock oscillator selected,
                    * the system clock frequency does not exceed the limit */
                    if ((((((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.ICK) > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
                       || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.FCK) > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
                       || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.PCKB) > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
                       || ((BSP_CFG_XTAL_HZ >> SYSTEM.SCKCR.BIT.PCKD) > LPC_MAX_MIDDLE_SPEED2_FREQ_HZ))
                    {
                        return LPC_ERR_ILL_MAIN_CLK_FREQ;
                    }
                break;
