...
sci_err_t sci_mcu_param_check(uint8_t const chan)
{
    ...
    if ((SCI_CH1 != chan) && (SCI_CH5 != chan) && (SCI_CH6 != chan)
        && (SCI_CH12 != chan))
    {
                         ...
                         void            *p_args)
{
    ...
    switch(cmd)
    {
        ...
        #if SCI_CFG_TEI_INCLUDED
        /* SCI_CMD_EN_TEI is obsolete command, but it exists only for compatibility with older version. */
        case (SCI_CMD_EN_TEI):
        {
            break;
        }
        ...
        case (SCI_CMD_GENERATE_BREAK):
        {
            ...
            if((SCI_DTC_ENABLE == hdl->rom->dtc_dmaca_tx_enable)
                || (SCI_DMACA_ENABLE == hdl->rom->dtc_dmaca_tx_enable))
            {
