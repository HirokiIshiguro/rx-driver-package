...
/* Defines for SCI_IIC support */
    ...
    #include "r_sci_iic_rx14t_private.h"
...
{
    BSP_LOCK_SCI1,
    (sci_regs_t)&SCI1, // Cast to SCI register type
    (volatile uint32_t R_BSP_EVENACCESS_SFR *)&SYSTEM.MSTPCRB.LONG, BIT30_MASK,  // Ensure proper access alignment
...
sci_iic_ch_ctrl_t g_sci_iic_ch1_ctrl =
{
    ...
    SCI_IIC_STS_NO_INIT,
    SCI_IIC_STS_NO_INIT,
    (uint8_t)INT_ZERO, // Simple I2C mode
    (sci_iic_info_t *)NULL
};
    #endif /* SCI_IIC_CFG_CH1_INCLUDED */
...
{
    BSP_LOCK_SCI5,
    (sci_regs_t)&SCI5, // Cast to SCI register type
    (volatile uint32_t R_BSP_EVENACCESS_SFR *)&SYSTEM.MSTPCRB.LONG, BIT26_MASK, // Ensure proper access alignment
...
sci_iic_ch_ctrl_t g_sci_iic_ch5_ctrl =
{
    ...
    #endif /* SCI_IIC_CFG_CH5_INCLUDED */
...
{
    BSP_LOCK_SCI6,
    (sci_regs_t)&SCI6, // Cast to SCI register type
    (volatile uint32_t R_BSP_EVENACCESS_SFR *)&SYSTEM.MSTPCRB.LONG, BIT25_MASK, // Ensure proper access alignment
...
sci_iic_ch_ctrl_t g_sci_iic_ch6_ctrl =
{
    ...
    #endif /* SCI_IIC_CFG_CH6_INCLUDED */
...
{
    BSP_LOCK_SCI12,
    (sci_regs_t)&SCI12, // Cast to SCI register type
    (volatile uint32_t R_BSP_EVENACCESS_SFR *)&SYSTEM.MSTPCRB.LONG, BIT4_MASK, // Ensure proper access alignment
...
sci_iic_ch_ctrl_t g_sci_iic_ch12_ctrl =
{
    ...
    #endif /* SCI_IIC_CFG_CH12_INCLUDED */
...
sci_iic_return_t r_sci_iic_check_arguments(sci_iic_info_t * p_sci_iic_info, sci_iic_api_mode_t called_api)
{
    ...
    if (((((((((((SCI_IIC_NUM_CH0 == p_sci_iic_info->ch_no) || (SCI_IIC_NUM_CH2 == p_sci_iic_info->ch_no))
            ...
            || (SCI_IIC_NUM_CH11 == p_sci_iic_info->ch_no)) || (SCI_IIC_NUM_CH12 < p_sci_iic_info->ch_no)))
    {
 ...
 * Arguments    : channel - Which channel to use.
 * Return Value : none
 **********************************************************************************************************************/
...
{
    R_BSP_VOLATILE_EVENACCESS uint8_t * const ppcr = \
            (uint8_t *)((uint32_t)SCI_IIC_PRV_PCR_BASE_REG + (uint32_t)port_gr); // Pointer to PCR register 

    R_BSP_VOLATILE_EVENACCESS uint8_t * const ppdr  = \
            (uint8_t *)((uint32_t)SCI_IIC_PRV_PDR_BASE_REG + (uint32_t)port_gr); // Pointer to PDR register

    R_BSP_VOLATILE_EVENACCESS uint8_t * const podr0 = \
            (uint8_t *)((uint32_t)SCI_IIC_PRV_ODR0_BASE_REG + (uint32_t)(port_gr * 2)); // Pointer to ODR0 register

    R_BSP_VOLATILE_EVENACCESS uint8_t * const podr1 = \
            (uint8_t *)((uint32_t)SCI_IIC_PRV_ODR1_BASE_REG + (uint32_t)(port_gr * 2)); // Pointer to ODR1 register
...
void r_sci_iic_int_enable(sci_iic_info_t * p_sci_iic_info)
{
    ...
    (*prom->pipr) = ipr_set_val; /* Sets interrupt source priority register. */
...
{
    R_BSP_VOLATILE_EVENACCESS uint8_t * const ppmr = \
            (uint8_t *)((uint32_t)SCI_IIC_PRV_PMR_BASE_REG + (uint32_t)port_gr); // Pointer to PMR register

    R_BSP_VOLATILE_EVENACCESS uint8_t * const ppfs = \
            (uint8_t *)((uint32_t)SCI_IIC_PRV_PFS_BASE_REG + (uint32_t)((port_gr * 8) + pin_num)); // Pointer to PFS register
