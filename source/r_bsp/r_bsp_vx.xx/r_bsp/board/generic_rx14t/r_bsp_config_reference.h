...
/* NOTE:
   The default settings are the same as when using MCB-RX14T.
...
/* Enter the product part number for your MCU. This information will be used to obtain information about your MCU such 
   ...
   R 5 F 51 4T 5 A G FM
   | | | |  |  | | | |  Macro Name                      Description
   | | | |  |  | | | |__BSP_CFG_MCU_PART_PACKAGE      = Package type, number of pins, and pin pitch
   | | | |  |  | | |____not used                      = Products with wide temperature range (D:-40-105C G:-40-125C)
   | | | |  |  | |______not used
...
/* Group name.
   ...
   4T           = RX14T Group
*/
#define BSP_CFG_MCU_PART_GROUP          "RX14T"
...
/* Clock source select (CKSEL).
   ...
   3 = Sub-Clock Oscillator (N/A for RX14T)
   4 = PLL Circuit
*/
...
/* The input clock frequency is specified and then the system clocks are set by specifying the multipliers used.
   The multiplier settings are used to set the clock registers in resetprg.c. If a 8MHz clock is used and the 
   ICLK is 48MHz, PCLKB is 48MHz, PCLKD is 48MHz, and FCLK is 48MHz, then the settings would be:
...
/* Main Clock Oscillator Wait Time (MOSCWTCR).
   ...
   0x00(00000b): Wait time = 0 cycles (0 us)
   0x01(00001b): Wait time = 1024 cycles (256 us)
   0x02(00010b): Wait time = 2048 cycles (512 us)
   0x03(00011b): Wait time = 4096 cycles (1.024 ms)
   0x04(00100b): Wait time = 8192 cycles (2.048 ms)
   0x05(00101b): Wait time = 16384 cycles (4.096 ms)
   0x06(00110b): Wait time = 32768 cycles (8.192 ms)
   0x07(00111b): Wait time = 65536 cycles (16.384 ms)
   0x08(01000b): Wait time =131072 cycles (32.768 ms)
...
*/
#define BSP_CFG_CONFIGURATOR_VERSION                (2240)
