...
{
    /* If false, NACK all Hot Join requests. */
    uint32_t hot_join_acknowledge                    : 1;

    /* Notify the application when an IBI Hot-Join request has been NACK'd. */
    uint32_t notify_rejected_hot_join_requests       : 1;

    /* Notify the application when an IBI Controller Role request has been NACK'd. */
    uint32_t notify_rejected_controllerrole_requests : 1;

    /* Notify the application when an IBI Interrupt request has been NACK'd. */
...
typedef struct s_target_command_response_info
{
    ...
    /* Max Data Speed Turnaround. */
    ri3c_clock_data_turnaround_t clock_data_turnaround;

    /* Enable transmission of the of the Max Read Max Read Turnaround Time. */
    bool                         read_turnaround_time_enable;

    /* Max Read Turnaround Time. */
    uint32_t                     read_turnaround_time;

    /** This byte represents the Target's internal oscillator frequency in increments of 0.5 MHz (500kHz),
     *  up to 127.5 MHz (See GETXTIME in the MIPI I3C Specification v1.1). */
...
/* Base register for this channel */
    ...
    uint32_t           device_index;                          // The device index selected using ri3c_api_t::deviceSelect.
    ri3c_bitrate_mode_t device_bitrate_mode;                  // Runtime bitrate settings to use for the next transfer.
    ri3c_target_info_t  current_target_info;                  // The last ri3c_target_info_t read during ENTDAA.
